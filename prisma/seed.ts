...
async function seedHiragana() {
  const hiragana = [
    ...
    { character: "け", romaji: "ke", displayOrder: 9, mnemonicHint: "🗝️ A key sticking out of a lock, shaped like a 'k' with an extra leg." },
    { character: "こ", romaji: "ko", displayOrder: 10, mnemonicHint: "〰️ Two parallel strokes stacked like two commas — 'ko-ko' for 'comma-comma'." },
    // S row
    ...
    { character: "せ", romaji: "se", displayOrder: 14, mnemonicHint: "⚖️ A seesaw shape tilting side to side — 'se' for 'seesaw'." },
    ...
    { character: "へ", romaji: "he", displayOrder: 29, mnemonicHint: "⛰️ A single upward slope like a small hill — 'he' for a hill." },
    ...
    { character: "め", romaji: "me", displayOrder: 34, mnemonicHint: "👁️ An eye shape looking back at you — 'me' means 'eye' in Japanese." },
    ...
    { character: "を", romaji: "wo", displayOrder: 45, mnemonicHint: "✍️ A fishhook with a flourish — rarely written, only used as the particle 'wo/o'." },
    { character: "ん", romaji: "n", displayOrder: 46, mnemonicHint: "〰️ A single hooked stroke — the only sound that's just 'n', hooked and alone." },
    // Dakuten G
    ...
    { character: "げ", romaji: "ge", displayOrder: 50, mnemonicHint: "🗝️✨ Same as け (ke) plus dakuten marks — the key-in-lock clicks 'ge'." },
    { character: "ご", romaji: "go", displayOrder: 51, mnemonicHint: "〰️✨ Same as こ (ko) plus dakuten marks — the two commas buzz into 'go'." },
    // Z row
    ...
    { character: "ぜ", romaji: "ze", displayOrder: 55, mnemonicHint: "⚖️✨ Same as せ (se) plus dakuten marks — the seesaw buzzes 'ze'." },
    ...
    { character: "べ", romaji: "be", displayOrder: 65, mnemonicHint: "⛰️✨ Same as へ (he) plus dakuten marks — the small hill buzzes 'be'." },
    ...
    { character: "ぺ", romaji: "pe", displayOrder: 70, mnemonicHint: "⛰️🔵 Same as へ (he) with a small circle (゜) — the hill pops into 'pe'." },
...
async function seedKatakana() {
  const katakana = [
    { character: "ア", romaji: "a", displayOrder: 1, mnemonicHint: "🅰️ A deformed capital A hides in the shape — tilt your head and connect the lines to see it." },
    { character: "イ", romaji: "i", displayOrder: 2, mnemonicHint: "🦅 An eagle perched on a branch — its legs and curved back form the イ shape." },
    { character: "ウ", romaji: "u", displayOrder: 3, mnemonicHint: "🪞 Looks just like hiragana う — similar enough to use directly as your memory hook." },
    { character: "エ", romaji: "e", displayOrder: 4, mnemonicHint: "🏗️ The cross-section of a steel girder an engineer uses — or elevator doors closed shut." },
    { character: "オ", romaji: "o", displayOrder: 5, mnemonicHint: "🎭 An opera singer with mouth wide open, belting 'Ohhh!' — that round O shape." },
    { character: "カ", romaji: "ka", displayOrder: 6, mnemonicHint: "🪞 Almost identical to hiragana か, just missing one small stroke — easy to remember by comparison." },
    { character: "キ", romaji: "ki", displayOrder: 7, mnemonicHint: "🔑 The top half of hiragana き, shaped like the teeth of a weird key." },
    { character: "ク", romaji: "ku", displayOrder: 8, mnemonicHint: "👨‍🍳 A tall cook's hat seen from the side — 'ku' for cook." },
    { character: "ケ", romaji: "ke", displayOrder: 9, mnemonicHint: "🔤 Looks just like the letter K — that's it, that's the whole hint!" },
    { character: "コ", romaji: "ko", displayOrder: 10, mnemonicHint: "📐 Two 90-degree corners stacked on top of each other — 'ko' for corners." },
    { character: "サ", romaji: "sa", displayOrder: 11, mnemonicHint: "🐟 A sardine and salmon skewered together — fish = さかな (sakana), so 'sa' for sakana." },
    { character: "シ", romaji: "shi", displayOrder: 12, mnemonicHint: "😶 A smiley face where both eyes are sideways and stacked — 'she' has a very weird face." },
    { character: "ス", romaji: "su", displayOrder: 13, mnemonicHint: "🦸 Superman's disembodied supersuit floating in the sky — where did he go naked?!" },
    { character: "セ", romaji: "se", displayOrder: 14, mnemonicHint: "🪞 Nearly identical to hiragana せ — use that familiarity to remember it." },
    { character: "ソ", romaji: "so", displayOrder: 15, mnemonicHint: "🧵 One vertical needle with a long thread for sewing — needles are vertical to stab straight down." },
    { character: "タ", romaji: "ta", displayOrder: 16, mnemonicHint: "🌮 Shaped like a kite — kites are たこ (tako) in Japanese. This one's a giant flying taco!" },
    { character: "チ", romaji: "chi", displayOrder: 17, mnemonicHint: "📣 A cheerleader in mid-cheer pose — 'chi' for cheerleader." },
    { character: "ツ", romaji: "tsu", displayOrder: 18, mnemonicHint: "🪡 Two vertical needles with thread — unlike シ (a face, horizontal), ツ has two needles pointing down." },
    { character: "テ", romaji: "te", displayOrder: 19, mnemonicHint: "📡 A telephone pole with its crossbars — 'te' for telephone." },
    { character: "ト", romaji: "to", displayOrder: 20, mnemonicHint: "🪆 A totem pole with one notch carved in — 'to' for totem." },
    { character: "ナ", romaji: "na", displayOrder: 21, mnemonicHint: "🐳 A majestic narwhal swimming up to the surface — 'na' for narwhal." },
    { character: "ニ", romaji: "ni", displayOrder: 22, mnemonicHint: "📏 Two needles lying flat on their sides, resting — no thread, just two parallel lines." },
    { character: "ヌ", romaji: "nu", displayOrder: 23, mnemonicHint: "🍜 Chopsticks pulling noodles up out of a bowl — 'nu' for noodles." },
    { character: "ネ", romaji: "ne", displayOrder: 24, mnemonicHint: "🐴 A horse leaping over a hurdle, shouting 'Neigh!' — 'ne' for neigh." },
    { character: "ノ", romaji: "no", displayOrder: 25, mnemonicHint: "👃 A single long nose — 'no' for nose." },
    { character: "ハ", romaji: "ha", displayOrder: 26, mnemonicHint: "👒 Shaped like a wide rice paddy hat — connect the two strokes to see the brim." },
    { character: "ヒ", romaji: "hi", displayOrder: 27, mnemonicHint: "😄 'He' is saying 'heehee' with a cheeky grin — you can see the smile in the curved stroke." },
    { character: "フ", romaji: "fu", displayOrder: 28, mnemonicHint: "🚩 A triangle-shaped flag waving in the wind — 'fu' for flag, hooray!" },
    { character: "ヘ", romaji: "he", displayOrder: 29, mnemonicHint: "🪞 Identical to hiragana へ — know one, know both!" },
    { character: "ホ", romaji: "ho", displayOrder: 30, mnemonicHint: "✝️ A holy cross radiating holy light — 'ho' for holy." },
    { character: "マ", romaji: "ma", displayOrder: 31, mnemonicHint: "📐 Look at all those angles, lengths, measurements — 'ma' for math!" },
    { character: "ミ", romaji: "mi", displayOrder: 32, mnemonicHint: "🚀 Three missiles flying straight toward you — be careful! 'mi' for missiles." },
    { character: "ム", romaji: "mu", displayOrder: 33, mnemonicHint: "🐄 A cow's friendly face with a round snout — Moo! 'mu' for moo." },
    { character: "メ", romaji: "me", displayOrder: 34, mnemonicHint: "✖️ An X crossed over 目 (め/me), which means 'eye' in Japanese — meh, now it's dead." },
    { character: "モ", romaji: "mo", displayOrder: 35, mnemonicHint: "🪞 Very similar to hiragana も — use that familiarity as your hook." },
    { character: "ヤ", romaji: "ya", displayOrder: 36, mnemonicHint: "🪞 Looks just like hiragana や, minus one small stroke — easy to remember." },
    { character: "ユ", romaji: "yu", displayOrder: 37, mnemonicHint: "🪝 'You' have a hook for a hand — what are you, a pirate?" },
    { character: "ヨ", romaji: "yo", displayOrder: 38, mnemonicHint: "🧘 Someone doing yoga — only a yogi could bend into such a boxy position." },
    { character: "ラ", romaji: "ra", displayOrder: 39, mnemonicHint: "🦖 A raptor wearing sweet sunglasses like a rapper — 'ra' for raptor." },
    { character: "リ", romaji: "ri", displayOrder: 40, mnemonicHint: "🪞 Very similar to hiragana り — use that to remember it." },
    { character: "ル", romaji: "ru", displayOrder: 41, mnemonicHint: "🛣️ Two routes you can take — Route one and Route two, splitting at the fork." },
    { character: "レ", romaji: "re", displayOrder: 42, mnemonicHint: "💁 Rei's beautiful red hair flowing downward — 're' for Rei." },
    { character: "ロ", romaji: "ro", displayOrder: 43, mnemonicHint: "🔲 A square road looping endlessly, never ending — 'ro' for road." },
    { character: "ワ", romaji: "wa", displayOrder: 44, mnemonicHint: "❓ Shaped like a question mark — questions often start with 'what.'" },
    { character: "ヲ", romaji: "wo", displayOrder: 45, mnemonicHint: "🐕 A dog woofing so hard its tongue flies out — 'wo' for woof." },
    { character: "ン", romaji: "n", displayOrder: 46, mnemonicHint: "👁️ A man with only one eye — horizontal = face like シ; diagonal = this one-eyed man." },
    { character: "ガ", romaji: "ga", displayOrder: 47, mnemonicHint: "🪞✨ Same as カ (ka) — like hiragana か — plus dakuten marks (゛), now buzzing 'ga'." },
    { character: "ギ", romaji: "gi", displayOrder: 48, mnemonicHint: "🔑✨ Same as キ (ki) — the key shape — plus dakuten marks, now 'gi'." },
    { character: "グ", romaji: "gu", displayOrder: 49, mnemonicHint: "👨‍🍳✨ Same as ク (ku) — the cook's hat — plus dakuten marks, now 'gu'." },
    { character: "ゲ", romaji: "ge", displayOrder: 50, mnemonicHint: "🔤✨ Same as ケ (ke) — the letter K — plus dakuten marks, now 'ge'." },
    { character: "ゴ", romaji: "go", displayOrder: 51, mnemonicHint: "📐✨ Same as コ (ko) — two corners — plus dakuten marks, now 'go'." },
    { character: "ザ", romaji: "za", displayOrder: 52, mnemonicHint: "🐟✨ Same as サ (sa) — the sakana skewer — plus dakuten marks, now 'za'." },
    { character: "ジ", romaji: "ji", displayOrder: 53, mnemonicHint: "😶✨ Same as シ (shi) — she's weird face — plus dakuten marks, now 'ji'." },
    { character: "ズ", romaji: "zu", displayOrder: 54, mnemonicHint: "🦸✨ Same as ス (su) — Superman's floating suit — plus dakuten marks, now 'zu'." },
    { character: "ゼ", romaji: "ze", displayOrder: 55, mnemonicHint: "🪞✨ Same as セ (se) — similar to hiragana せ — plus dakuten marks, now 'ze'." },
    { character: "ゾ", romaji: "zo", displayOrder: 56, mnemonicHint: "🧵✨ Same as ソ (so) — the sewing needle — plus dakuten marks, now 'zo'." },
    { character: "ダ", romaji: "da", displayOrder: 57, mnemonicHint: "🌮✨ Same as タ (ta) — the flying taco kite — plus dakuten marks, now 'da'." },
    { character: "デ", romaji: "de", displayOrder: 58, mnemonicHint: "📡✨ Same as テ (te) — the telephone pole — plus dakuten marks, now 'de'." },
    { character: "ド", romaji: "do", displayOrder: 59, mnemonicHint: "🪆✨ Same as ト (to) — the totem pole — plus dakuten marks, now 'do'." },
    { character: "バ", romaji: "ba", displayOrder: 60, mnemonicHint: "👒✨ Same as ハ (ha) — the rice paddy hat — plus dakuten marks, now 'ba'." },
    { character: "ビ", romaji: "bi", displayOrder: 61, mnemonicHint: "😄✨ Same as ヒ (hi) — the heehee face — plus dakuten marks, now 'bi'." },
    { character: "ブ", romaji: "bu", displayOrder: 62, mnemonicHint: "🚩✨ Same as フ (fu) — the triangle flag — plus dakuten marks, now 'bu'." },
    { character: "ベ", romaji: "be", displayOrder: 63, mnemonicHint: "🪞✨ Same as ヘ (he) — identical to hiragana へ — plus dakuten marks, now 'be'." },
    { character: "ボ", romaji: "bo", displayOrder: 64, mnemonicHint: "✝️✨ Same as ホ (ho) — the holy cross — plus dakuten marks, now 'bo'." },
    { character: "パ", romaji: "pa", displayOrder: 65, mnemonicHint: "👒🔵 Same as ハ (ha) — the rice paddy hat — plus a small circle (゜), now popping 'pa'." },
    { character: "ピ", romaji: "pi", displayOrder: 66, mnemonicHint: "😄🔵 Same as ヒ (hi) — the heehee face — plus a small circle (゜), now 'pi'." },
    { character: "プ", romaji: "pu", displayOrder: 67, mnemonicHint: "🚩🔵 Same as フ (fu) — the triangle flag — plus a small circle (゜), now 'pu'." },
    { character: "ペ", romaji: "pe", displayOrder: 68, mnemonicHint: "🪞🔵 Same as ヘ (he) — identical to hiragana へ — plus a small circle (゜), now 'pe'." },
    { character: "ポ", romaji: "po", displayOrder: 69, mnemonicHint: "✝️🔵 Same as ホ (ho) — the holy cross — plus a small circle (゜), now 'po'." },
  ];
